...
def merge_files(features_file, cds_file_zip, protein_file_zip):
    ...
    # genes
    gene = features_data[[(features_data["# feature"] == 'gene') and (features_data["class"] == 'protein_coding')]]
    cds = features_data[[(features_data["# feature"] == 'CDS') and (features_data["class"] == 'with_protein')]]
    gene_join = gene.merge(cds, on='locus_tag')

    # pseudogenes
    pseudo_gene = features_data[[(features_data["# feature"] == 'gene') and (features_data["class"] == 'pseudogene')]]
    pseudo_cds = features_data[[(features_data["# feature"] == 'CDS') and (features_data["class"] == 'without_protein')]]
